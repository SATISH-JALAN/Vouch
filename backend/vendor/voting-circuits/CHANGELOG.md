...
- Added `prepare_delegation_proving`, `prepare_vote_proof_proving`, and
  `prepare_share_reveal_proving` to build halo2 fixed-base commitment tables.
  The existing `warm_*_keys` helpers now prepare proving automatically.
- Prepared proving reduced measured delegation proof time by up to 31% on
  narrow Rayon pools, at a retained-memory cost of about 26 MiB. Wide pools
  continue to use planned multiexp; the LRZ backend returns `Ok(false)`.
- Preparation is idempotent and single-flighted. It does not change proofs,
  verifying keys, public inputs, or interoperability with older provers and
  verifiers.
