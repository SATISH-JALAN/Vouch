...
export async function GET() {
  const upstream = process.env.POF_ATTEST_URL?.replace(/\/$/, '')
  ...
  if (upstream) {
    try {
      ...
      attestor = res.ok ? { ok: true, pubkey: body.pubkey } : { ok: false, message: `attestor answered HTTP ${res.status}` }
      demoProver = res.ok && Boolean(body.demoProver)
    } catch (err) {
  ...
  const solana =
    SOLANA_RPC_URL && POF_POOL && RELAYER_SECRET_KEY && BORROWER_SECRET_KEY
      ...
      : null
  const status: ServiceStatus = { attestor, demoProver, solana }
